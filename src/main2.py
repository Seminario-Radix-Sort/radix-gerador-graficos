...
for ordenacao in ordenacoes:
    for tipo in tipos:
        ...
        plot.set_xlabel('Tamanho da Entrada (n)', fontsize=25, color='black')
        plot.set_ylabel('Tempo de Execução (s)', fontsize=25, color='black')
        legend = plt.legend(
            title='Linguagem',
            loc='upper left',
            ncol=1,
        ...
        plt.savefig(f"datasets/graficosB/grafico-{ordenacao}{tipo}.png")
        plt.close()
...
for tipo in tipos:
    ...
    plot.set_xlabel('Valor', fontsize=25, color='black')
    plot.set_ylabel('Aparições', fontsize=25, color='black')
    ...
    plt.savefig(f"datasets/graficosB/dispersao-1000000{tipo}.png")
    plt.close()
